...
function manageMedia(messageId) {
    ...
    fetch(`actions/messagesfundation/get-message-media.php?message_id=${messageId}`)
...
function switchMediaTab(type) {
    ...
    // Recargar archivos
    if (currentMessageId) {
        fetch(`actions/messagesfundation/get-message-media.php?message_id=${currentMessageId}`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
...
function deleteMedia(mediaId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch('actions/messagesfundation/delete-media.php', {
                method: 'POST',
                headers: {
...
function uploadFiles(files) {
    ...
    xhr.open('POST', 'actions/messagesfundation/upload-media.php', true);
    xhr.send(formData);
}
</script>